...
export default function App() {
  ...
  // persistAccounts/persistProducts accept either a full array (old behavior) OR an
  // updater function (fresh) => newArray. The updater form re-reads the latest data
  // from Supabase first, so a stale local `accounts`/`products` value (e.g. from a
  // slow tab, or two people editing at once) can't silently wipe out data that was
  // added by someone else in the meantime.
  const persistAccounts = async (n) => {
    if (typeof n === "function") {
      const fresh = (await storageGet(KEYS.accounts, accounts)) || accounts;
      const next = n(fresh);
      setAccounts(next);
      await storageSet(KEYS.accounts, next);
      return next;
    }
    setAccounts(n); await storageSet(KEYS.accounts, n); return n;
  };
  ...
  const persistProducts = async (n) => {
    if (typeof n === "function") {
      const fresh = (await storageGet(KEYS.products, products)) || products;
      const next = n(fresh);
      setProducts(next);
      await storageSet(KEYS.products, next);
      return next;
    }
    setProducts(n); await storageSet(KEYS.products, n); return n;
  };
  const quickAddProduct = async (fields) => {
    const newProduct = { ...fields, id: uid("prod") };
    await persistProducts((fresh) => [newProduct, ...fresh]);
      ...
      <main className="app-main" style={styles.main}>
        ...
        {tab === "inventory" && (
          <Inventory
            ...
            onAdd={async (p) => { await persistProducts((fresh) => [{ ...p, id: uid("prod") }, ...fresh]); showToast("Product added"); }}
            onEdit={async (id, updates) => { await persistProducts((fresh) => fresh.map((p) => (p.id === id ? { ...p, ...updates } : p))); showToast("Product updated"); }}
            onDelete={async (id) => { await persistProducts((fresh) => fresh.filter((p) => p.id !== id)); showToast("Product deleted"); }}
            onImport={async (rows) => {
              const withIds = rows.map((p) => ({ ...p, id: uid("prod") }));
              await persistProducts((fresh) => [...withIds, ...fresh]);
        ...
        {tab === "bills" && (
          <Bills
            ...
            onAdd={async (bill, journalEntry, qtyChanges) => {
              ...
              await persistProducts((fresh) => fresh.map((p) => {
        ...
        {tab === "invoices" && (
          <Invoices
            ...
            onEdit={async (invoiceId, updatedInvoice, cogsJournalEntry, oldQtyChanges, newQtyChanges) => {
              ...
              if (oldQtyChanges.length || newQtyChanges.length) {
                await persistProducts((fresh) => {
                  let nextProducts = fresh;
          ...
          <Expenses
            ...
            onBulkImport={async ({ newExpenses, newJournalEntries, newAccounts }) => {
              if (newAccounts.length) {
                await persistAccounts((fresh) => [...fresh, ...newAccounts]);
              }
              await persistJournal([...newJournalEntries, ...journal]);
        ...
        {tab === "income" && (
          <IncomeTab
            ...
            onAddCategory={async (acc) => { const withId = { ...acc, id: uid("acc") }; await persistAccounts((fresh) => [...fresh, withId]); return withId; }}
        ...
        {tab === "accounts" && (
          <ChartOfAccounts
            ...
            onAdd={async (acc) => { await persistAccounts((fresh) => [...fresh, { ...acc, id: uid("acc") }]); showToast("Account added"); }}
            onAddMany={async (accs) => { const withIds = accs.map((a) => ({ ...a, id: uid("acc") })); await persistAccounts((fresh) => [...fresh, ...withIds]); showToast(`${accs.length} accounts added`); }}
            onEdit={async (id, updates) => { await persistAccounts((fresh) => fresh.map((a) => (a.id === id ? { ...a, ...updates } : a))); showToast("Account updated"); }}
            onDelete={async (id) => { await persistAccounts((fresh) => fresh.filter((a) => a.id !== id)); showToast("Account deleted"); }}
...
function Invoices({ accounts, products, invoices, currentUser, canEdit, onAdd, onEdit, onRecordPayment, onEditPayment, onDeletePayment, onDelete, onBulkImport, onQuickAddProduct }) {
  ...
  const cancelForm = () => {
    ...
    setDiscount(""); setNote(""); setDeposit(""); setDepositAccount("");
  };
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(20,20,20,0.5)", zIndex: 320, display: "flex", alignItems: "flex-start", justifyContent: "center", overflowY: "auto", padding: "40px 16px" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.cardSolid, border: `1px solid ${PALETTE.line}`, borderRadius: 20, maxWidth: 440, width: "100%", padding: 28, boxShadow: "0 30px 80px rgba(0,0,0,0.5)" }}>
        ...
        {cashAccounts.length === 0 ? (
          <div style={{ padding: 14, background: PALETTE.debitSoft, borderRadius: 12, marginBottom: 10 }}>
            <div style={{ fontSize: 13, color: PALETTE.debit, fontWeight: 600, marginBottom: 4 }}>No Cash/Bank/bKash account found</div>
            <div style={{ fontSize: 12.5, color: PALETTE.inkSoft }}>Go to Chart of Accounts and check that at least one Asset account (like "Cash in Hand") exists. Then come back and try again.</div>
          </div>
        ) : (
          <form onSubmit={submit}>
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(20,20,20,0.45)", zIndex: 300, display: "flex", alignItems: "flex-start", justifyContent: "center", overflowY: "auto", padding: "40px 16px" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: PALETTE.cardSolid, border: `1px solid ${PALETTE.line}`, borderRadius: 20, maxWidth: 560, width: "100%", padding: 28, boxShadow: "0 30px 80px rgba(0,0,0,0.5)" }}>
        ...
        {invoice.note && (
          <div style={{ marginBottom: 18, padding: 12, background: PALETTE.chip, borderRadius: 12 }}>
            <div style={{ fontSize: 11, color: PALETTE.inkSoft, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.3, marginBottom: 4 }}>Note</div>
            <div style={{ fontSize: 13, whiteSpace: "pre-wrap" }}>{invoice.note}</div>
          </div>
        )}
